...
p_age_phase <- ggplot(data, aes(x = StudyPhase, y = Age_years)) +
  ...
  geom_boxplot(width = .1, outlier.shape = NA) +
  geom_jitter(width = .05, alpha = .001) +
  labs(title = "Age Distribution (Stratified by Educational Stage)")+
  xlab('Educational Stage')+ylab('Age in years')+theme_bruce()+
  theme(panel.grid.major.y = element_line(linetype = 'dashed',linewidth = 0.8),
        axis.text.x = element_text(angle = -30))
...
p_age_grade <- ggplot(data, aes(x = Grade, y = Age_years)) +
  ...
  geom_boxplot(width = .1, outlier.shape = NA) +
  geom_jitter(width = .05, alpha = .001) +
  labs(title = "Age Distribution (Stratified by Grade)")+
  xlab('Grade')+ylab('Age in years')+theme_bruce()+
  theme(panel.grid.major.y = element_line(linetype = 'dashed',linewidth = 0.8))+
