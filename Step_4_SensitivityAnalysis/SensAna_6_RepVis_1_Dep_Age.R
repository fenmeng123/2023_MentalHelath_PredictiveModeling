...
FileList_MdlRes <- list.files(
  path = "../Res_2_Results/ResSensAna_AgeStratified_Dep/",
  pattern = "Res.*xlsx",
  full.names = T)
...
p <- AllRes %>%
  ...
  scale_x_continuous(limits = c(85.5, 87.5)) +  # 设置Y轴的值范围
  # labs(title = 'Predicting Depression Age stratified by One-year old')+
  xlab('Balanced ACC (%)')+
  ylab('Age (years old)')+
...
p
ggsave('../Res_2_Results/ResSensAna_AgeStratified_Dep/SensRes_BalanACC_Age_Dep.svg',
       width = 6,
       height = 6,
       plot = p)
...
ggsave('../Res_2_Results/ResSensAna_AgeStratified_Dep/SensRes_Coef_Age_Dep.svg',
       width = 10,
       height = 8,
       plot = p)
...
ggsave('../Res_2_Results/ResSensAna_AgeStratified_Dep/SensRes_CoefErrorBar_Age_Dep.svg',
       width = 10,
       height = 8,
       plot = p)