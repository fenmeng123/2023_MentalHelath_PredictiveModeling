...
Created on Wed Mar 27 13:33:01 2024
@author: Kunru Song
"""
import pandas as pd
...
class Mdl_Pipe:
    ...
    def MdlFit(self,train,method='weighting'):
        ...
        if method == 'weighting':
            ...
            self.pipe['classifier'].set_params(**para)
            downsamp_train_pos = resample(train_pos,\
                                          replace=False,\
                                          random_state=1,\
                                          n_samples=Num_NegCase)
            train = pd.concat([downsamp_train_pos,train_pos],ignore_index=True)
            train_x = train[self.feature_pool]
            train_y = (train[self.Y_Name]).values.ravel()
        elif method == 'upsampling':
            para = {'class_weight':None}
            self.pipe['classifier'].set_params(**para)
            upsamp_train_neg = resample(train_neg,\
                                        replace=True,\
                                        random_state=1,\
                                        n_samples=abs(Num_NegCase - Num_PosCase))
            train = pd.concat([train_pos,upsamp_train_neg],ignore_index=True)
            train_x = train[self.feature_pool]
            train_y = (train[self.Y_Name]).values.ravel()
    ...
    def MdlRepeat(self,Num_repeat,test):
        ...
        if self.fit_method == 'downsampling':
            for i in range(Num_repeat):
                ...
                downsamp_train_pos = resample(train_pos,\
                                              replace=False,\
                                              n_samples=self.Num_NegCase)
                train = pd.concat([downsamp_train_pos,train_neg],ignore_index=True)
            ...
            for i in range(Num_repeat):
                ...
                upsamp_train_neg = resample(train_neg,\
                                            replace=True,\
                                            n_samples=abs(self.Num_NegCase - self.Num_PosCase))
                train = pd.concat([upsamp_train_neg,train_pos],ignore_index=True)
                IterRes = self.RndSamp_TrainPred(train,test,i,IterRes)
                # Test_y_proba.insert(i,'PosCase_Iter_'+str(i),self.pred_proba[:,1])
                if i == 0:
                    MdlCoef.insert(0, 'CoefName', self.pipe['classifier'].feature_names_in_)
