# R Script - Subfunctions
# Performing C/IER Identification on Representative Sample Data (RPSD)
# It should be noted that the training data, named internal validation sample in
# this manuscript is not a actual national representative sample. Abbreviation 
# "RPSD" used here just aim to make a different naming for the two data sample.
# 
# Kunru Song 2023.3
...
IdentifyCarelessResp_P2 <- function(data){
  ...
  # careless::mahad use the function psych::outlier to calculate Mahalanobis distance.
  # The return value is the D2 values for each case
  cat('------End-Auto-Quality-Control-Part2------\n')
  return(as.data.table(data))
}

plot.box.distribution <- function(var, file_path, title, thresh_line) {  
  # 创建一个数据框，用于绘制箱线图和直方图  
  df <- data.frame(value = var)  
  
  # 创建箱线图  
  boxplot_ggplot <- ggplot(df, aes(x = value)) +  
    geom_boxplot() +  
    labs(title = paste('Boxplot of', title, sep = ' ')) +  
    theme_bruce()  
  
  # 创建直方图
  if (length(thresh_line)==2){
    histogram_ggplot <- ggplot(df, aes(x = value)) +  
      geom_histogram(bins = 50) +  
      geom_vline(xintercept = thresh_line[1], color = 'lightblue', linetype = 'dashed', size = 1) +
      geom_vline(xintercept = thresh_line[2], color = 'lightblue', linetype = 'dashed', size = 1) +
      labs(title = paste('Histogram of', title, sep = ' ')) +  
      theme_bruce()  
  }else{
    histogram_ggplot <- ggplot(df, aes(x = value)) +  
      geom_histogram(bins = 50) +  
      geom_vline(xintercept = thresh_line, color = 'lightblue', linetype = 'dashed', size = 1) +  
      labs(title = paste('Histogram of', title, sep = ' ')) +  
      theme_bruce()  
  }
  # 将箱线图和直方图保存到PNG文件中  
  file_png <- paste0(file_path, ".png")  
  ggsave(file_png, arrangeGrob(boxplot_ggplot, histogram_ggplot, nrow = 1),   
         width = 11.25, height = 4.5, dpi = 300)  
  
  # 将箱线图和直方图保存到SVG文件中  
  file_svg <- paste0(file_path, ".svg")  
  ggsave(file_svg, arrangeGrob(boxplot_ggplot, histogram_ggplot, nrow = 1),   
         width = 11.25, height = 4.5)  
}
